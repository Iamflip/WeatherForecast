...
namespace MetricsManager.Jobs
{
    ...
    {
        private IRepositoryMM<DotNetMetric> _metricRepository;
        private IAgentsRepository<AgentInfo> _agentRepository;
        private IMetricsAgentClient _client;
        public DotNetMetricJob(IRepositoryMM<DotNetMetric> metricRepository, IAgentsRepository<AgentInfo> agentsRepository, IMetricsAgentClient client)
        {
        ...
        public Task Execute(IJobExecutionContext context)
        {
            ...
            for (int i = 0; i < agents.Count; i++)
            {
                ...
                var result = _client.GetFromToDotNetMetrics(request);
